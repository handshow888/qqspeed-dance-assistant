...
第三行左侧的 `RECORD ON/OFF` 是数据集录制开关。它可以在停止、运行或暂停状态下直接用鼠标点击，并会立即写入 `config.json`。开启后仍然只在识别运行期间、符合下文事件触发条件时保存样本；关闭后不再保存新样本。

录制开关右侧的 `YOLO ON/OFF` 用于切换箭头识别后端。它只在识别状态为 `STOPPED` 时允许点击：开启后使用训练得到的 YOLO 模型识别 8 类箭头，关闭后立即恢复传统 OpenCV 模板识别。运行或暂停过程中不能切换，避免两个识别器的连续帧结果混入同一轮。节奏条、滑块和光标始终继续使用 OpenCV。
