...
当前版本已支持“传统四键”和“双人舞蹈”的经典/焕新 UI。双人舞蹈复用传统四键的箭头、滑块和节奏条识别规则，仅使用独立的固定光标位置。飞车舞蹈的选择入口与独立配置已建立，但在对应素材和规则补齐前不会允许启动识别。
...
光标不再识别。固定基准线由当前玩法/UI 的模式 JSON 中 `space.cursor_window_ratio` 单独配置，不再放在总 `config.json` 中。传统四键经典/焕新分别为 `0.618` 和 `0.603646`；双人舞蹈的初始值分别为 `0.608` 和 `0.593646`，比对应的传统四键向左约一个窗口宽度百分点。自动 ROI 始终使用完整窗口宽度，因此窗口移动或等比例缩放后仍然有效。
